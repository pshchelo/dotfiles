...
if [ -n "${ACTION}" ];then
  ...
  Shutdown)
    ...
    ## or via ConsoleKit
    dbus-send --system --dest=org.freedesktop.ConsoleKit.Manager /org/freedesktop/ConsoleKit/Manager org.freedesktop.ConsoleKit.Manager.Stop
    ;;
  Reboot)
    #zenity --question --text "Are you sure you want to reboot?" && gksudo reboot
    ## Or via ConsoleKit
    dbus-send --system --dest=org.freedesktop.ConsoleKit.Manager /org/freedesktop/ConsoleKit/Manager org.freedesktop.ConsoleKit.Manager.Restart
    ;;
  Suspend)
    #gksudo pm-suspend
    ...
    ;;
  LockScreen)
    #`slock` OR `gnome-screensaver-command -l` OR `xdg-screensaver lock`
